...
library GovernorQuorumLogicV4 {
  ...
  /// @notice Retrieves the latest quorum numerator using the GovernorClockLogicV4 library.
  /// @param self The storage structure containing the quorum numerator history.
  /// @return The latest quorum numerator.
  function quorumNumerator(GovernorStorageTypesV4.GovernorStorage storage self) public view returns (uint256) {
   ...
   */
  function quorum(GovernorStorageTypesV4.GovernorStorage storage self, uint256 timepoint) public view returns (uint256) {
  ...
  function updateQuorumNumerator(
    ...
    self.quorumNumeratorHistory.push(GovernorClockLogicV4.clock(self), SafeCast.toUint208(newQuorumNumerator));
  ...
  function quorumReached(
    ...
    return
      quorum(self, GovernorProposalLogicV4._proposalSnapshot(self, proposalId)) <= self.proposalTotalVotes[proposalId];
  }
}
