...
import { GovernorGovernanceLogicV4 } from "./GovernorGovernanceLogicV4.sol";
import { GovernorFunctionRestrictionsLogicV4 } from "./GovernorFunctionRestrictionsLogicV4.sol";
import { SafeCast } from "@openzeppelin/contracts/utils/math/SafeCast.sol";
import { Address } from "@openzeppelin/contracts/utils/Address.sol";
import { DoubleEndedQueue } from "@openzeppelin/contracts/utils/structs/DoubleEndedQueue.sol";

/// @title GovernorProposalLogicV4
...
library GovernorProposalLogicV4 {
   ...
   */
  function canProposalStartInNextRound(GovernorStorageTypesV4.GovernorStorage storage self) external view returns (bool) {
  ...
  function queue(
    ...
    GovernorStateLogicV4.validateStateBitmap(
      self,
      proposalId,
      GovernorStateLogicV4.encodeStateBitmap(GovernorTypesV4.ProposalState.Succeeded)
    );
  ...
  function execute(
    ...
    GovernorStateLogicV4.validateStateBitmap(
      ...
      proposalId,
      GovernorStateLogicV4.encodeStateBitmap(GovernorTypesV4.ProposalState.Succeeded) |
        GovernorStateLogicV4.encodeStateBitmap(GovernorTypesV4.ProposalState.Queued)
    );
    ...
    if (GovernorGovernanceLogicV4.executor(self) != contractAddress) {
    ...
    if (GovernorGovernanceLogicV4.executor(self) != contractAddress && !self.governanceCall.empty()) {
  ...
  function cancel(
    ...
    GovernorStateLogicV4.validateStateBitmap(
      self,
      proposalId,
      GovernorStateLogicV4.ALL_PROPOSAL_STATES_BITMAP ^
        GovernorStateLogicV4.encodeStateBitmap(GovernorTypesV4.ProposalState.Canceled) ^
        GovernorStateLogicV4.encodeStateBitmap(GovernorTypesV4.ProposalState.Executed)
    );

    if (account == proposalProposer(self, proposalId)) {
      require(
        GovernorStateLogicV4._state(self, proposalId) == GovernorTypesV4.ProposalState.Pending,
  ...
  function _propose(
    ...
    uint256 depositThresholdAmount = GovernorDepositLogicV4._depositThreshold(self);
    ...
    if (depositAmount > 0) {
      GovernorDepositLogicV4.depositFunds(self, depositAmount, proposer, proposalId);
    }
  ...
  function validateProposeParams(
    ...
    if (self.proposals[proposalId].roundIdVoteStart != 0) {
      ...
      revert GovernorUnexpectedProposalState(proposalId, GovernorStateLogicV4._state(self, proposalId), bytes32(0));
    }

    GovernorFunctionRestrictionsLogicV4.checkFunctionsRestriction(self, targets, calldatas);
  }
  ...
  function _executeOperations(
    ...
    self.timelock.executeBatch{ value: msg.value }(
      ...
      calldatas,
      0,
      GovernorGovernanceLogicV4.timelockSalt(descriptionHash, contractAddress)
    );
    // cleanup for refund
    delete self.timelockIds[proposalId];
  }
  ...
  function _queueOperations(
    ...
    bytes32 salt = GovernorGovernanceLogicV4.timelockSalt(descriptionHash, contractAddress);
   ...
   * @return The proposal id.
   */
  function _cancel(GovernorStorageTypesV4.GovernorStorage storage self, uint256 proposalId) private returns (uint256) {
  ...
  function _canProposalStartInNextRound(
    ...
    uint48 currentBlock = GovernorClockLogicV4.clock(self);
  ...
  function _proposalSnapshot(
    ...
    if (currentRoundDeadline <= GovernorClockLogicV4.clock(self)) {
      currentRoundDeadline = GovernorClockLogicV4.clock(self);
    }
