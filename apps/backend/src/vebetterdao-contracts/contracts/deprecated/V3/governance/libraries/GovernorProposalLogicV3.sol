...
import { GovernorGovernanceLogicV3 } from "./GovernorGovernanceLogicV3.sol";
import { GovernorFunctionRestrictionsLogicV3 } from "./GovernorFunctionRestrictionsLogicV3.sol";
import { SafeCast } from "@openzeppelin/contracts/utils/math/SafeCast.sol";
import { Address } from "@openzeppelin/contracts/utils/Address.sol";
import { DoubleEndedQueue } from "@openzeppelin/contracts/utils/structs/DoubleEndedQueue.sol";

/// @title GovernorProposalLogicV3
...
library GovernorProposalLogicV3 {
   ...
   */
  function canProposalStartInNextRound(GovernorStorageTypesV3.GovernorStorage storage self) external view returns (bool) {
  ...
  function queue(
    ...
    GovernorStateLogicV3.validateStateBitmap(
      self,
      proposalId,
      GovernorStateLogicV3.encodeStateBitmap(GovernorTypesV3.ProposalState.Succeeded)
    );
  ...
  function execute(
    ...
    GovernorStateLogicV3.validateStateBitmap(
      ...
      proposalId,
      GovernorStateLogicV3.encodeStateBitmap(GovernorTypesV3.ProposalState.Succeeded) |
        GovernorStateLogicV3.encodeStateBitmap(GovernorTypesV3.ProposalState.Queued)
    );
    ...
    if (GovernorGovernanceLogicV3.executor(self) != contractAddress) {
    ...
    if (GovernorGovernanceLogicV3.executor(self) != contractAddress && !self.governanceCall.empty()) {
  ...
  function cancel(
    ...
    GovernorStateLogicV3.validateStateBitmap(
      self,
      proposalId,
      GovernorStateLogicV3.ALL_PROPOSAL_STATES_BITMAP ^
        GovernorStateLogicV3.encodeStateBitmap(GovernorTypesV3.ProposalState.Canceled) ^
        GovernorStateLogicV3.encodeStateBitmap(GovernorTypesV3.ProposalState.Executed)
    );

    if (account == proposalProposer(self, proposalId)) {
      require(
        GovernorStateLogicV3._state(self, proposalId) == GovernorTypesV3.ProposalState.Pending,
  ...
  function _propose(
    ...
    uint256 depositThresholdAmount = GovernorDepositLogicV3._depositThreshold(self);
    ...
    if (depositAmount > 0) {
      GovernorDepositLogicV3.depositFunds(self, depositAmount, proposer, proposalId);
    }
  ...
  function validateProposeParams(
    ...
    if (self.proposals[proposalId].roundIdVoteStart != 0) {
      ...
      revert GovernorUnexpectedProposalState(proposalId, GovernorStateLogicV3._state(self, proposalId), bytes32(0));
    }

    GovernorFunctionRestrictionsLogicV3.checkFunctionsRestriction(self, targets, calldatas);
  }
  ...
  function _executeOperations(
    ...
    self.timelock.executeBatch{ value: msg.value }(
      ...
      calldatas,
      0,
      GovernorGovernanceLogicV3.timelockSalt(descriptionHash, contractAddress)
    );
    // cleanup for refund
    delete self.timelockIds[proposalId];
  }
  ...
  function _queueOperations(
    ...
    bytes32 salt = GovernorGovernanceLogicV3.timelockSalt(descriptionHash, contractAddress);
   ...
   * @return The proposal id.
   */
  function _cancel(GovernorStorageTypesV3.GovernorStorage storage self, uint256 proposalId) private returns (uint256) {
  ...
  function _canProposalStartInNextRound(
    ...
    uint48 currentBlock = GovernorClockLogicV3.clock(self);
  ...
  function _proposalSnapshot(
    ...
    if (currentRoundDeadline <= GovernorClockLogicV3.clock(self)) {
      currentRoundDeadline = GovernorClockLogicV3.clock(self);
    }
