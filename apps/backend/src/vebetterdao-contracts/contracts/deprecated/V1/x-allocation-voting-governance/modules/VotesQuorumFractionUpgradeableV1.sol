 ...
 * @dev Extension of {XAllocationVotingGovernorV1} for voting weight extraction from an {ERC20Votes} token and a quorum expressed as a
 * fraction of the total supply.
 */
abstract contract VotesQuorumFractionUpgradeableV1 is Initializable, VotesUpgradeableV1 {
  using Checkpoints for Checkpoints.Trace208;

  /// @custom:storage-location erc7201:b3tr.storage.XAllocationVotingGovernorV1.VotesQuorumFraction
  struct VotesQuorumFractionStorage {
    Checkpoints.Trace208 _quorumNumeratorHistory;
  }

  // keccak256(abi.encode(uint256(keccak256("b3tr.storage.XAllocationVotingGovernorV1.VotesQuorumFraction")) - 1)) & ~bytes32(uint256(0xff))
